...
import t from"cheerio";import e from"axios";let handler=async(t,{conn:e,text:r})=>{if(!r)return t.reply("Y el texto?");let n=await e.getName(t.sender);encodeURIComponent(r);let a=t.reply(MultiNK.Proces(n));await a;try{let l=await fuentes(r),o=`*[_>] Estilos de texto para:* _${r}_
